...
public class Injector {
    ...
    public static mate.academy.car.lib.Injector getInstance(String mainPackageName) {
        if (injectors.containsKey(mainPackageName)) {
            return injectors.get(mainPackageName);
        }
        mate.academy.car.lib.Injector injector = new mate.academy.car.lib
    ...
    public Object getInstance(Class<?> certainInterface) {
        ...
        for (Field field : declaredFields) {
            ...
            if (field.getDeclaredAnnotation(mate.academy.car.lib.Inject.class) != null) {
        ...
        for (Class<?> clazz : classes) {
            ...
            for (Class<?> singleInterface : interfaces) {
                if (singleInterface.equals(certainInterface)
                        && (clazz.isAnnotationPresent(mate.academy.car.lib.Service.class)
                        || clazz.isAnnotationPresent(mate.academy.car.lib.Dao.class))) {
                    return clazz;
                }
            }
        }
